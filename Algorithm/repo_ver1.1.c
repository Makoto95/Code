...
int main(){
  ...
  int store = 0;
  /*----ready index----*/
  element_t fhIndex[PRIME1][PRIME2];
  element_t lhIndex[PRIME1][PRIME2];
  for( i = 0; i < PRIME1; i++){
    for(j = 0; j<PRIME2; j++){
      elementInit(&fhIndex[i][j]);
      elementInit(&lhIndex[i][j]);
    }
  }
  ...
  while(1){
    ...
    element_t* actElement = &fhIndex[hash1st][hash2nd];
...
/*====Last half create Index====*/
  while(1){
    ...
    if(searchAndFlagIndex(&fhIndex[hash1st][hash2nd], &s[start], wordLen) == 0){
      /*----linear search Array addres----*/
      element_t* actElement = &lhIndex[hash1st][hash2nd];
      if(actElement->counter == 0){
        storeWord(actElement, &s[start], wordLen);
        store++;
      }
...
/*====Last half create Index end====*/
  ...
  element_t* sortArray[PRIME1*PRIME2];
  n = 0;
  for(i=0; i<PRIME1; i++){
    for(j=0; j<PRIME2; j++){
      if(fhIndex[i][j].counter > 0 && fhIndex[i][j].countFlag == TRUE){
        sortArray[n] = &fhIndex[i][j];
        n++;
      }
      if(fhIndex[i][j].next != NULL){
        element_t* actElement = fhIndex[i][j].next;
        while(1){
  ...
  for(i=0; i<PRIME1; i++){
    for(j=0; j<PRIME2; j++){
      if(lhIndex[i][j].counter > 0){
        sortArray[n] = &lhIndex[i][j];
        n++;
      }
      if(lhIndex[i][j].next != NULL){
        element_t* actElement = lhIndex[i][j].next;
        while(1){
          if(actElement->counter > 0){
            sortArray[n] = actElement;
            n++;
...
inline uint_fast64_t getHash(char* addr, int_fast32_t start, int_fast32_t end){
  ...
  while(i <= end){
    hash = hash * PRIME3 + tolower(addr[i]);
    i++;
  }
  return hash;
}
