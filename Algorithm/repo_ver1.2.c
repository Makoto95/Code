...
int main(){
  ...
  int store = 0;
  /*----ready index----*/
  element_t fhIndex[PRIME1][PRIME2][PRIME3];
  element_t lhIndex[PRIME1][PRIME2][PRIME3];
  for( i = 0; i < PRIME1; i++){
    for(j = 0; j<PRIME2; j++){
      for (k = 0; k < PRIME3; k++) {
        elementInit(&fhIndex[i][j][k]);
        fhIndex[i][j][k].mallocFlag = FALSE;
        elementInit(&lhIndex[i][j][k]);
        lhIndex[i][j][k].mallocFlag = FALSE;
      }
    }
  }
  ...
  uint32_t hash1st, hash2nd, hash3rd;
  ...
  while(1){
    ...
    hash2nd = (uint_fast32_t)(hashBase % PRIME2);
    hash3rd = (uint_fast32_t)(hashBase % PRIME3);

    /*----linear search Array addres----*/
    element_t* actElement = &fhIndex[hash1st][hash2nd][hash3rd];
  ...
  while(1){
    ...
    hash2nd = (uint32_t)(hashBase % PRIME2);
    hash3rd = (uint32_t)(hashBase % PRIME3);

    if(searchAndFlagIndex(&fhIndex[hash1st][hash2nd][hash3rd], &s[start], wordLen) == 0){
      /*----linear search Array addres----*/
      element_t* actElement = &lhIndex[hash1st][hash2nd][hash3rd];
      if(actElement->counter == 0){
        storeWord(actElement, &s[start], wordLen);
        store++;
      }
  ...
  element_t* sortArray[PRIME1*PRIME2*PRIME3];
  n = 0;
  for(i=0; i<PRIME1; i++){
    for(j=0; j<PRIME2; j++){
      for(k=0; k<PRIME3; k++){
        if(fhIndex[i][j][k].counter > 0 && fhIndex[i][j][k].countFlag == TRUE){
          sortArray[n] = &fhIndex[i][j][k];
          n++;
        }
        if(fhIndex[i][j][k].next != NULL){
          element_t* actElement = fhIndex[i][j][k].next;
          while(1){
    ...
    for(j=0; j<PRIME2; j++){
      for(k=0; k<PRIME3; k++){
        if(fhIndex[i][j][k].next != NULL){
          ...
          while(1){
            int flag=0;
            nextElement = actElement->next;
            if(nextElement == NULL) flag = 1;
            free(actElement);
            actElement = NULL;
  ...
  for(i=0; i<PRIME1; i++){
    for(j=0; j<PRIME2; j++){
      for(k=0; k<PRIME3; k++){
        if(lhIndex[i][j][k].counter > 0){
          sortArray[n] = &lhIndex[i][j][k];
          n++;
        }
        if(lhIndex[i][j][k].next != NULL){
          element_t* actElement = lhIndex[i][j][k].next;
          while(1){
            if(actElement->counter > 0){
              sortArray[n] = actElement;
              n++;
