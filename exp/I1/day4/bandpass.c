...
int filter(complex double* Y, int b, int t, double n){
  double Omegab = 2 * PI * b;
  double Omegat = 2 * PI * t;
  int i;
  int Nhalf = ((int)n%2==0) ? (n / 2) : (int)(n/2+0.5);
  double omegab = Omegab / 44100;
  double omegat = Omegat / 44100;
  for(i=0; 2*PI*(double)i/n < omegab; i++){
    Y[i] = 0;
    Y[(int)n-i] = 0;
  }
  for(i = (int)(omegat*n/(2*PI)); i<Nhalf; i++){
    Y[i] = 0;
    Y[(int)n-i] = 0;
  }
  return 0;
...
int main(int argc, char ** argv) {
  ...
  while (1) {
    ...
    /* pass frequency bandpassfilter*/
    filter(Y, bottom, top, n);
